...
export function AsciiCanvas({
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      await Promise.all(
        srcList.map(async (s) => {
          const frames = await decodeGif(s, cols, rows, speed);
          if (!cancelled) allFrames.current.set(s, frames);
        }),
      );
      if (cancelled) return;

      activeSrc.current = srcList[0];
      frameIdx.current = 0;
      startLoop();
    };

    const pickOtherSrc = (): string => {
      if (srcList.length === 1) return srcList[0];
      let next = activeSrc.current;
      while (next === activeSrc.current) {
        next = srcList[Math.floor(Math.random() * srcList.length)];
      }
      return next;
    };
    ...
    const drawPass = (
      ...
      for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < cols; col++) {
          ...
          if (dither > 0)
            luma = Math.min(255, luma + bayerThreshold(col, row) * dither);
          ...
          ctx.fillStyle = injectAlpha(passColor, alpha);
    ...
    const renderFrame = (imageData: ImageData) => {
      ...
      const resolvedColor = resolveColor(color, 1);
      ...
      if (isGlitching) {
        ...
        drawPass(ctx, data, true, 'rgba(255,20,70,0.4)', -shift);
        drawPass(ctx, data, true, 'rgba(20,100,255,0.4)', shift);
        ctx.globalCompositeOperation = 'source-over';
        drawPass(ctx, data, true, resolvedColor, 0);
      } else {
        drawPass(ctx, data, false, resolvedColor, 0);
      }
    ...
    const startLoop = () => {
      const advance = () => {
        const frames = allFrames.current.get(activeSrc.current);
...
function resolveColor(color: string, alpha: number): string {
  if (!color.startsWith('var(')) return applyAlpha(color, alpha);
  const match = color.match(/var\((--[^)]+)\)/);
  if (!match) return `rgba(0,255,136,${alpha})`;
  const val = getComputedStyle(document.documentElement)
    .getPropertyValue(match[1])
    .trim();
  return applyAlpha(val || '#00ff88', alpha);
}
